...
const Pagination = ({ totalPages, currentPage }: { totalPages: number; currentPage: number }) => {
    ...
    const getVisiblePages = () => {
        ...
        return Array.from({ length: Math.min(4, totalPages - startPage + 1) }, (_, index) => startPage + index);
    };
            ...
            <div className='flex justify-center gap-2'>
                {getVisiblePages().map((page) => (
                    <Button
                        ...
                        className={currentPage === page ? "bg-btnblue text-white hover:bg-btnblue/80" : "border-gray1 text-btnblue"}
